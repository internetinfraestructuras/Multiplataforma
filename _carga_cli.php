...
if($_SESSION['USER_LEVEL']==0) {
    ...
    if (isset($_POST['idcliente']) && $_POST['idcliente'] != '') {
        $result = $util->selectJoin("clientes", $campos, "LEFT JOIN municipios ON municipios.id = clientes.localidad LEFT JOIN provincias ON provincias.id=clientes.provincia", $orden, 'clientes.id = ' . $_POST['idcliente']);
        ...
        $result = $util->selectJoin("clientes", $campos, "LEFT JOIN municipios ON municipios.id = clientes.localidad LEFT JOIN provincias ON provincias.id=clientes.provincia", $orden,$where);
    }
}else{ // no es root
    ...
    if (isset($_POST['idcliente']) && $_POST['idcliente'] != '') {
        ...
        $result = $util->selectJoin("clientes", $campos, "LEFT JOIN municipios ON municipios.id = clientes.localidad LEFT JOIN provincias ON provincias.id=clientes.provincia", $orden, $where . ' and user_create in (select id from usuarios where revendedor = '.$_SESSION['REVENDEDOR'].')');
    }
