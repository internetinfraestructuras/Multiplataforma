...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"Página",rows:"rows", sort: "sidx",order: "sord", search:"_search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
...
$.jgrid.extend({
	...
	delRowData : function(rowid, tabla) {
		var success = false, rowInd, ia;
		var respuesta = confirm("¿ Seguro/a de querer borrar este registro ?");
       	// llamo al fichero encargado de borrar el registro, este fichero comienza por borrar- y le agrego el sufijo en funcion de la tabla de la que quiero borrar
        if(respuesta) {
            $.ajax({
                url: 'php/borrar-' + tabla + '.php',
                type: 'POST',
                cache: false,
                async: true,
                data: {id: rowid, oper: 'del' + rowid},
