...
<SCRIPT LANGUAGE="Javascript">
	...
	function confirmacion_total(troncal,posicion)
	{

		opener.location=('lista_salida_confirmar_eliminacion_num.php?troncal='+troncal+'&posicion='+posicion); 
		self.close();

	}

	function cancelacion_total(troncal,callcenter)
	{

		opener.location=('lista_salida_eliminacion_num_cancelada.php?troncal='+troncal+'&callcenter='+callcenter); 
		self.close();
...
<body>
					...
					if($usuario!=$superuser_propietario_legitimo)
					{
						...
						$web_violacion="eliminacion_tarifa_popup.php";
					...
					/*$sel = "select descripcion from tarifas_prueba where usuario_troncal='$troncal' and prefijo='$prefijo'";
					$query = mysql_query($sel) or die(mysql_error());
					...
					$sel = "select nombre from centralitas where id_centralita=(select id_centralita from troncales where usuario_troncal='$troncal')";
					$query = mysql_query($sel) or die(mysql_error());
						
					while ($row = mysql_fetch_assoc($query)) 
					{
						$nombre_centralita=$row['nombre'];
					}
					...
					<?php echo "<p>¿Confirma que desea eliminar el numérico <b>$numero</b> asociado a la troncal <b>$troncal</b> ?</p>";
					...
					<input type="button" value="Eliminar" onclick="confirmacion_total('<?php echo $troncal;?>','<?php echo $posicion;?>')" style=" width:60;height:20; background-image:url(./images/bg_button.gif)"> 
					<input type="button" value="Cancelar" onclick="cancelacion_total('<?php echo $troncal;?>','<?php echo $callcenter;?>')" style=" width:60;height:20; background-image:url(./images/bg_button.gif)"> 
