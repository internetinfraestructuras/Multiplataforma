...
//echo $query;
    ...
    public function update($tabla, $campos, $valor, $where, $log=true){
        ...
        for ($i = 0; $i < count($campos); $i++) {
            $query.= $this->cleanstring($campos[$i]) . "='". $this->cleanstring($valor[$i]) . "',";
        }
...
//echo $query;
        if (!($result = $link->query($query)))
            throw new Exception('Error en count.');
...
//var_dump($result);
    ...
    function check_token($id, $token){
        return true;
        $r= $this->selectWhere('usuarios',array('id'), 'hash="'.$token.'"');

        $r1 = mysqli_fetch_array($r);
        if(intval($r1[0])==intval($id))
            return true;
        else
            return false;
