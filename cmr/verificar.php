...
if (isset($_REQUEST['id_orden'])) {
    ...
    $result= $util->selectJoin('aprovisionados',array('num_pon'), ' JOIN ordenes on ordenes.id_provision=aprovisionados.id','',' ordenes.id='.$_REQUEST['id_orden']);
...
//InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.LastConnectionError ERROR_AUTHENTICATION_FAILURE
/*
 * InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANIPConnection.1.LastConnectionError ERROR_NONE Refresh
InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.LastConnectionError ERROR_AUTHENTICATION_FAILURE
InternetGatewayDevice.WANDevice.1.WANConnectionDevice.2.WANIPConnection.1.LastConnectionError ERROR_NONE
InternetGatewayDevice.WANDevice.1.WANConnectionDevice.3.WANIPConnection.1.LastConnectionError ERROR_NO_CARRIER
InternetGatewayDevice.WANDevice.1.WANConnectionDevice.4.WANIPConnection.1.LastConnectionError ERROR_NO_CARRIER
...
//print_r($json);
    ...
    $internet_pppoe=$json[0]['InternetGatewayDevice']['WANDevice'][1]['WANConnectionDevice'][2]['WANIPConnection'][1]['ConnectionStatus']['_value'];$n++;
    $internet_ip=$json[0]['InternetGatewayDevice']['WANDevice'][1]['WANConnectionDevice'][1]['WANPPPConnection'][1]['ConnectionStatus']['_value'];$n++;
    $vozip=$json[0]['InternetGatewayDevice']['WANDevice'][1]['WANConnectionDevice'][3]['WANIPConnection'][1]['ConnectionStatus']['_value'];$n++;
    $tv=$json[0]['InternetGatewayDevice']['WANDevice'][1]['WANConnectionDevice'][4]['WANIPConnection'][1]['ConnectionStatus']['_value'];$n++;
    $acceso_pppoe=$json[0]['InternetGatewayDevice']['Services']['VoiceService'][1]['VoiceProfile'][1]['Line'][1]['X_HW_LastRegisterError']['_value'];$n++;
    $acceso_voz=$json[0]['InternetGatewayDevice']['Services']['VoiceService'][1]['VoiceProfile'][1]['Line'][1]['X_HW_LastRegisterError']['_value'];$n++;

    if($internet_pppoe=='Connected' || $internet_ip[0]=='Connected')
    ...
    $aItem = array(
        ...
        'television' => $status_tv,
        'telefono' => $status_tel,
        'registro_pppoe' => $acceso_pppoe,
        'registro_vozip' => $acceso_voz
    );
